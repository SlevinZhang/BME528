...
class Model_Fuji_Synapse_3_1_1_1 extends Model_Fuji {
	...
	public function _patient_studies()
	{
		...
		$query = "SELECT ACCESSION, DESCRIPTION, BODY_PART, TO_CHAR(STUDY_TIMEDATE, 'MON DD HH24:MI') AS TIME, MODALITY, INSTANCE_EUID FROM (
				SELECT ROW_NUMBER() OVER (ORDER BY MAX(STUDY.STUDY_TIMEDATE) DESC) AS R, STUDY.INSTANCE_EUID,
				NVL(STUDY.RIS_STUDY_EUID,'N/A') AS ACCESSION, MAX(MODALITY.NAME) AS MODALITY,
				'[' || SUBSTR(STUDY_STATUS.STATUS,1,1) || '] ' || MAX(PROCEDURE_INFO.DESCRIPTION) AS DESCRIPTION,
				REPLACE(MAX(BODY_PART.NAME),'Unknown','-') AS BODY_PART, MAX(STUDY.STUDY_TIMEDATE) AS STUDY_TIMEDATE
				FROM STUDY 
	...
	{
		return (file_exists(Mnode::$DIR_DUMP.$study)
							 AND filesize(Mnode::$DIR_DUMP.$study) > $img->OFFSET) ?
							 Mnode::$DIR_DUMP.$study : $img->FILENAME;
	}
	...
	public function _worklist()
	{
		...
		else 
		{
			...
			$query = 'SELECT ACCESSION, DESCRIPTION, BODY_PART, TO_CHAR(STUDY_TIMEDATE, \'YYYY-MM-DD HH24:MI:SS\') AS TIME, MODALITY, INSTANCE_EUID FROM (
					SELECT ROW_NUMBER() OVER (ORDER BY MAX(STUDY.STUDY_TIMEDATE) DESC) AS R, STUDY.INSTANCE_EUID, NVL(STUDY.RIS_STUDY_EUID,\'N/A\') AS ACCESSION,
					MAX(MODALITY.NAME) AS MODALITY, \'[\' || SUBSTR(STUDY_STATUS.STATUS,1,1) || \'] \' || MAX(PROCEDURE_INFO.DESCRIPTION) AS DESCRIPTION,
					NVL(REPLACE(MAX(BODY_PART.NAME),\'Unknown\',\'-\'),\'-\') AS BODY_PART, MAX(STUDY.STUDY_TIMEDATE) AS STUDY_TIMEDATE
					FROM STUDY
					LEFT JOIN STUDY_STATUS ON STUDY_STATUS.CODE = STUDY.STATUS
					LEFT JOIN PROCEDURE_INFO ON PROCEDURE_INFO.ID = STUDY.PROCEDURE_INFO_UID
					LEFT JOIN MODALITY ON MODALITY.ID = PROCEDURE_INFO.MODALITY_UID 
