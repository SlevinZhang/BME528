...
class Mnode_Core {
	...
	public static function parse_directory($directory,$id)
	{
...
//			echo 'Processing '.$directory.DIRECTORY_SEPARATOR.$file->getFilename()."\n";
			...
			if ($file->isFile())
			{
				...
				{
					$patientid = Automodeler::factory('PatientID')->load(db::select()->where('dcm_PatientID', '=', trim($id)));
				}
				//if ($count == 4)
					//exit;
				if ($patientid->loaded())
				...
				else
				{
					...
					if(!$dicom->PatientID)
					{
					$patientid->dcm_PatientID = $id;
					$patientid->dcm_PatientName = $id;
					}
					try
					{
						$patientid->save();
				...
				if ( ! $study->loaded())
				{
					...
					if(!$dicom->PatientID)
					{
					$study->dcm_PatientID = $id;
					$study->dcm_PatientName = $id;
					}
					try
					{
						$study->save();
				...
				if ( ! $series->loaded())
				{
					...
					if(!$dicom->PatientID)
					{
					$series->dcm_PatientID = $id;
					$series->dcm_PatientName = $id;
					}
					try
					{
						$series->save();
				...
				if ( ! $image->loaded())
				{
					...
					if(!$dicom->PatientID)
					{
					$image->dcm_PatientID = $id;
					$image->dcm_PatientName = $id;
					}
					try
					{
						$image->save();
