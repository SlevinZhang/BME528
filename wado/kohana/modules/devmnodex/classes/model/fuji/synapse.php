...
class Model_Fuji_Synapse extends Model_Fuji {
	...
	public function study()
	{
		...
		if ( ! file_exists($meta_file) OR ! file_exists($docs_file))// OR ! file_exists($exam_file))
		{
			...
			*/
			$query = "SELECT IMAGE, MODALITY, BOOKMARK, SERIES_UID, OBJECT_UID, REMOTE_AE, FILENAME, IMAGE_COUNT, DESCRIPTION, OFFSET, LENGTH, ROW_COUNT, COLUMN_COUNT FROM (
				SELECT ROW_NUMBER() OVER (ORDER BY IMAGE.ID ASC) AS IMAGE, MODALITY.NAME AS MODALITY, IMAGE.BOOKMARK AS BOOKMARK, SERIES.INSTANCE_EUID AS SERIES_UID, IMAGE.SOP_INSTANCE_EUID AS OBJECT_UID,  IMAGE.REMOTE_AE_UID AS REMOTE_AE, 		
				CONCAT(STORAGE.UNC_PATH, IMAGE_VERSION.FILENAME) AS FILENAME, IMAGE.ROW_COUNT, IMAGE.COLUMN_COUNT,
				IMAGE_VERSION.ID AS IMAGE_VERSION, SERIES.IMAGE_COUNT, NVL(SERIES.SERIES_DESC,'-') AS DESCRIPTION, STORAGE.EUID, IMAGE_VERSION.OFFSET, IMAGE_VERSION.LENGTH 
				FROM STUDY 
				LEFT JOIN PROCEDURE_INFO ON PROCEDURE_INFO.ID = STUDY.PROCEDURE_INFO_UID
