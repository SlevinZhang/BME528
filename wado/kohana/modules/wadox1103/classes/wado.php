...
//TODO:
//use MD5 hash for image file, info file
...
class Wado {
	...
	{
		//json_decode(file_get_contents(Mnode::$DIR_META.'img-'.$this->studyUID.Mnode::$TXT), TRUE);
		$images = Mnode::read_file('meta', 'img-'.$this->studyUID);
		$total_images = count($images);
		
		for ($i = 0; $i < $total_images; $i++)
		{
			if ($images[$i]['SOPInstanceUID'] == $this->objectUID)
			{
				$this->_metadata = $images[$i];
				$this->index = $i;
				break;
			}
		}
		
		//$this->_exam_metadata = json_decode(file_get_contents(Mnode::$DIR_EXAM.$this->studyUID.Mnode::$TXT));
		$this->_exam_metadata = Mnode::read_file('exam', $this->studyUID);
	}
	...
	function prepare_statement()
	{
		...
		{
			// Cropping
			$this->region = Mnode::crop_parameters($this->_exam_metadata['site'].$this->_metadata['REMOTE_AE']);
		}
		...
		$command .= ($this->imageQuality != '')? ' +Jq '.$this->imageQuality.' ' : '';
		$this->_default_cmd .= ($this->imageQuality != '')? ' +Jq '.$this->imageQuality.' ' : '';
