 ...
 *
 * @package Automodeler_MPTT
 * @author Mathew Davies
 * @author Kiall Mac Innes
 * @author Paul Banks
 * @author Nano Documet
 */
abstract class Automodeler_ORM_MPTT extends Automodeler_ORM
{
	/**
	 * @access public
	...
	/**
	 * @access public
	 * @var string Model holding the parent.
	 */
	...
	{
		// Initialize Automodeler (this will call _init() in the model)
		parent::__construct($id);
		
		// If any of the MPTT fields haven't been defined, create defaults
	...
	{
		Database::instance()->query(NULL, 'LOCK TABLE '.$this->_table_name.' WRITE', TRUE);
	}
	...
	/**
	 * Is the current node the first child of the supplied node?
	 *
	 * @access public
	 * @param Automodeler_MPTT $target Target
	 * @return bool
	 */
	public function is_first_child($target)
	{
		// Check if the current object is the first child of the given target.
		return (
					$this->is_child($target)
					AND ($this->{$this->left_column} - $target->{$this->left_column} == 1)
				);
	}

	/**
	 * Is the current node the last child of the supplied node?
	 *
	 * @access public
	 * @param Automodeler_MPTT $target Target
	 * @return bool
	 */
	public function is_last_child($target)
	{
		// Check if the current object is the last child of the given target.
		return (
					$this->is_child($target)
					AND ($this->{$this->right_column} + 1 == $target->{$this->right_column})
				);
	}
	...
	public function is_descendant($target)
	{
		return (
					...
					AND ($this->{$this->level_column} > $target->{$this->level_column})
					AND ($this->{$this->scope_column} == $target->{$this->scope_column})
				);
	}
	
	/**
	 * Is the current node an ancestor (parent or any level up) of the supplied node?
	 *
	 * @access public
	 * @param Automodeler_MPTT $target Target
	 * @return bool
	...
	{
		return (
					$this->is_descendant($target)
					AND ($this->{$this->level_column} == $target->{$this->level_column} + 1)
				);
	}
	...
	public function is_sibling($target)
	{
		...
		return ($this->parent()->id === $target->parent()->id);
	}
	 ...
	 *
	 * @access protected
	 * @return Automodeler_MPTT|FALSE on invalid scope
	 */
	public function root($scope = NULL)
	{
		...
		return Automodeler_MPTT::factory($this->model_name())->load(db::select_array($this->fields())->where($this->left_column, '=', 1)
	 ...
	 *
	 * @access public
	 * @param Automodeler_MPTT $parent Parent
	 * @return Automodeler_MPTT|FALSE on invalid scope
	 */
	public function set_as_parent(Automodeler_ORM_MPTT $parent)
	{
		if ($this->is_child($parent))
		{
			$this->_parent = $parent;
			return $this;
		}
	 ...
	 *
	 * @access public
	 * @return Automodeler_MPTT
	 */
	public function parent()
	{
		if ($this->_parent instanceof $this)
			return $this->_parent;

		return $this->parents(FALSE, TRUE, 'ASC', TRUE);
	}
	 ...
	 * @return Automodeler_MPTT
	 */
	public function parents($self = FALSE, $root = TRUE, $direction = 'ASC', $direct_parent_only = FALSE,
		$limit = FALSE, $levels = FALSE, Database_Query_Builder_Select $query = NULL)
	{
		$left_operator = $self ? '<=' : '<';
		$right_operator = $self ? '>=' : '>';
		...
		$query->where($this->left_column, $left_operator, $this->{$this->left_column})
		...
		if ($levels)
		{
			$query->where($this->level_column, '>=', $this->{$this->level_column} - $levels);
		}
		
		$parents =  Automodeler_MPTT::factory($this->model_name())->load($query, $limit);
	 ...
	 * @param bool $self include the current loaded node?
	 * @param string $direction direction to order the left column by.
	 * @param bool $direct_children_only return only direct children.
	 * @param bool $leaves_only return only descendants that are leaves.
	 * @param bool|integer $limit false to unlimeted results or integer to limit the number of results.
	 * @return Automodeler_MPTT
	 */
	public function descendants($self = FALSE, $direction = 'ASC', $direct_children_only = FALSE, $leaves_only = FALSE,
		$limit = FALSE, $levels = FALSE, Database_Query_Builder_Select $query = NULL)
	{		
		$left_operator = $self ? '>=' : '>';
		$right_operator = $self ? '<=' : '<';
		...
		return Automodeler_MPTT::factory($this->model_name())->load($query, $limit);
	}
	...
	public function siblings($self = FALSE, $direction = 'ASC',
		$limit = FALSE, Database_Query_Builder_Select $query = NULL)
	{	
		if ($query == NULL)
		{
			$query = db::select_array(array_keys($this->_data));
		...
		$query->where($this->left_column, '>', $this->parent()->{$this->left_column})
	 ...
	 * 
	 * Other object fields must be set in the normal Automodeler way 
	 ...
	 * Insert the object
	 * 
	 * Automodeler_MPTT|mixed $target target node primary key value or Automodeler_MPTT object. 
	...
	protected function insert($target, $copy_left_from, $left_offset, $level_offset)
	{
		...
		{
			$target = Automodeler_MPTT::factory($this->model_name())
				->load(db::select_array($this->fields())->where('id', '=', $target));
	 ...
	 *
	 * @access public
	 * @param Automodeler_MPTT|mixed $target target node primary key value or Automodeler_MPTT object.
	 * @return Automodeler_MPTT
	 */
	 ...
	 *
	 * @access public
	 * @param Automodeler_MPTT|integer $target target node id or Automodeler_MPTT object.
	 * @return Automodeler_MPTT
	 */
	 ...
	 * 
	 * @access public
	 * @return Automodeler_MPTT|bool 
	 * @throws Validation_Exception
	 */
	public function create()
	{
		// Don't allow creation directly as it will invalidate the tree
		throw new Automodeler_Exception('You cannot use create() on Automodeler_MPTT model :name. Use an appropriate insert_* method instead',
				array(':name' => get_class($this)));
	}
	
	public function set_validation($validation = NULL)
	{
	...
	public function delete(Database_Query_Builder_Delete $query = NULL)
	{
		if ($query !== NULL)
		{
			throw new Automodeler_Exception('Automodeler_MPTT does not support passing a query object to delete()');
		}
	...
	public function selected_fields()
	{
		...
		$fields = array();
		foreach ($this->_selected_fields as $key)
			$fields[] = $this->_table_name.'.'.$key;
		
		var_dump($this->_selected_fields);
		var_dump($fields);
		return $fields;
	}
	...
	*/
	...
	protected function move($target, $left_column, $left_offset, $level_offset, $allow_root_target)
	{
		...
		// Make sure we have the most up to date version of this AFTER we lock
		$this->lock();
		$this->reload();
		...
		try
		{
			...
			// Update the values.
			//Database::instance($this->_db)->query(NULL, 'UPDATE '.$this->_table_name.' 
			Database::instance()->query(NULL, 'UPDATE '.$this->_table_name.' 
	...
	*/
	 ...
	 * 
	 * This function's speed is irrelevant - its really only for debugging and unit tests
	 * 
	...
	private function get_scopes()
	{
		...
		// used by verify_tree()
		return DB::select()->as_object()->distinct($this->scope_column)->from($this->_table_name)->execute($this->_db);
	}
	...
	public function verify_scope($scope)
	{
		...
		$result = Database::instance($this->_db)->query(Database::SELECT, 'SELECT count(*) as count FROM `'.$this->_table_name.'` 
	...
	public function reload()
	{
		...
		return $this->set_fields($mptt_vals);
	}
