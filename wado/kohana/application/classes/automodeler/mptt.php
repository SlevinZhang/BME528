 ...
 * @author Nano Documet
 */
class Automodeler_MPTT extends Automodeler
{
	/**
	 * @access public
	...
	{
		Database::instance($this->_db)->query(NULL, 'LOCK TABLE '.$this->_table_name.' WRITE', TRUE);
	}
	...
	{
		return ($this->id === $target->parent->{$this->pk()});
	}
	...
	public function root($scope = NULL)
	{
		...
		$model = str_replace('Model_', '', get_class($this));
		
		return Automodeler_MPTT::factory($model)->load(db::select_array($this->fields())->where($this->left_colum, '=', 1)
	...
	public function parents($root = TRUE, $direction = 'ASC', $direct_parent_only = FALSE)
	{
		...
		$parents =  Automodeler_MPTT::factory($model)->load($query, $limit);
	...
	{		
		...
		return Automodeler_MPTT::factory($model)->load($query, $limit);
	}
	...
	{	
		...
		return Automodeler_MPTT::factory($model)->load($query, NULL);
	}
	...
	protected function insert($target, $copy_left_from, $left_offset, $level_offset)
	{
		...
		if ( ! $target instanceof $this)
		{
			...
			$target = Automodeler_MPTT::factory($model)->load(db::select_array($this->fields())->where($this->id, '=', $target));
	...
	protected function move($target, $left_column, $left_offset, $level_offset, $allow_root_target)
	{
		...
		try
		{
			if ( ! $target instanceof $this)
			{
				...
				$target = Automodeler_MPTT::factory($model)
					->load(db::select_array($this->fields())->where($this->id, '=', $target));
