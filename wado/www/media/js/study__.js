...
(function($){
	...
	function activateDrag()
	{
		$('#menu li a').click(function(event) {
			...
			if ($(this).hasClass('series')) {
				var side = ($('#vHalfR').hasClass('full')) ? 'vHalfR' : 'vHalfL';
				loadSeries($(this), side);
			}
			return false;
		});
		$('.drag').click(function(){return false;}).drag('start', function( ev, dd ){
				//$(dd.available).find('.wrapper').addClass('active');
				var top = ev.pageY + 30;
				var left = ev.pageX + 30;
				//alert(top + ' ' + left);
				return $( this ).clone()
					.css("opacity", .75 ).addClass('abs').width(260).addClass('layer9') //.addClass('abs').css({top:top, left:left})
			...
			.drag('end', function( ev, dd ){
				//$(dd.available).find('.wrapper').removeClass('active');
				$( dd.proxy ).remove();
				loadSeries($(this).find('a'), $(dd.drop).attr('id'));
			});
		$('.drop').drop(function(){
	...
	$('.half').hover(
		function() {
			if ($(this).find('.totalImages').html() != '') $(this).find('.nav, .preset').show();
			$(this).find('.togglePanes').show();
		},
		function() {
			$(this).find('.togglePanes, .nav, .preset').hide();
			$(this).removeData('status');
		}
	);

	$('.half').mousedown( function(e) {
		$(this).data('status', 'active');
		$(this).data('x', e.pageX);
		$(this).data('y', e.pageY);
		//$('#debug').show().html(e.pageX);
		return false;
	});
	
	$('.half').mouseup( function(e) {
		$(this).removeData('status');
		if ($('#main').data('mode') == 'imgZoom' || $('#main').data('mode') == 'imgWindow')
		{
			$(this).data('cols', $(this).find('.activeImage').width());
			updateImages(this, 21, 0);
		}
		
		return false;
	});

	$('.half').mousemove( function(e) {
		...
		if ($(this).data('status') == 'active')
		{
			if ($('#main').data('mode') == 'imgWindow')
			{
				...
				var wW	   = $(this).data('wWidth');
				var wC	   = $(this).data('wCenter');
				var index = parseInt($(this).find('.totalImages').text()) - 1;
				var cur    = $(this).find('img').eq(index);
				wW -= ($(this).data('y') - e.pageY);
				wC -= ($(this).data('x') - e.pageX);
				//$('#debug').show().html(series.seriesImages[series.index]+'&columns='+$(cur).width()+'&windowCenter='+wC+'&windowWidth='+wW);
				$(cur).attr('src', $(this).data('src')+'&columns='+$(cur).width()+'&windowCenter='+wC+'&windowWidth='+wW);
				$(this).data('wWidth', wW).data('wCenter', wC);
				/*
				var series = $(this).data('series');
				var img = '';
				for(var i=0; i<series.frames;i++)
				{
					//alert(seriesResponse.ratio[i]+ ' ' + imagesW + ' ' + imagesH + ' ' + (imagesW/imagesH));
					// First option is for wide images
					var vars = '&columns='+$(cur).width()+'&windowCenter='+wC+'&windowWidth='+wW;
					imgClass = (i == index)? '' : ' none';
					img += '<img class="center rel'+imgClass+'" onerror="this.onerror=null;this.src=\'img/blank.jpg\';" src="'+series.seriesImages[i]+vars+'" />';
					//img += '<img class="center rel'+imgClass+'" onerror="this.onerror=null;this.src=this.src;" src="'+seriesResponse.seriesImages[i]+dims+'" />';				
					
					//tmpImage = new Image();
					//tmpImage.src = seriesResponse.seriesImages[i]+dims;
				}	
				$(this).find('.images').html(img);
				*/
			}

			if ($('#main').data('mode') == 'imgPan')
			{
				var top  = parseInt($(this).find('img.activeImage').css('top'));
				var left = parseInt($(this).find('img.activeImage').css('left'));
				top = isNaN(top) ? 0 : top;
				left = isNaN(left) ? 0 : left;

				top  = top - (parseInt($(this).data('y')) - e.pageY);
				left = left - (parseInt($(this).data('x')) - e.pageX);
				$(this).find('img').css({top:top, left:left});
			}
			if ($('#main').data('mode') == 'imgZoom')
			{
				...
				var w = $(this).find('img.activeImage').width();
				var h = $(this).find('img.activeImage').height();
				var ratio = w/h;
				var pw = $(this).closest('.half').width();
				//var ph = $(this).closest('.half').height();
				var diff = (parseInt($(this).data('y')) - e.pageY);
				//diff = diff; - (diff % 2);
				//$('#debug').show().html(diff+' '+w+' '+left+' '+top+' '+h);
				//w = w - diff;
				h = h - diff;
				//left += diff;
				top += diff/2;
				left += diff/2;
				//$('#debug').append(';'+w+' '+left+' '+top+' '+w*ratio);
				$(this).data('cols', h*ratio);
				$(this).find('img').height(h).css({top:top, left:left});
			}
			// Setting the src of the current image, should be faster for window/level
			//var index = parseInt($(this).find('.totalImages').text()) - 1;
			//var cur    = $(this).find('img').eq(index);
			//$(this).data('src', $(cur).attr('src');
	...
	$('.togglePanes').click( function(){
		if ($(this).text() == 'Full')
		{
			$(this).text('Half');
			var previousW = $(this).closest('.half').width();
			$('.half').hide();
			$(this).closest('.half').show().removeClass('size1of2').addClass('full');
			var newW = $(this).closest('.half').width();
			var left = parseInt($(this).closest('.half').find('.activeImage').css('left'));
			left += (newW - previousW)/2;
			// Current image first
			$(this).closest('.half').find('.activeImage').css('left', left);
			// The rest
			$(this).closest('.half').find('.images img').each(function(index){
				$(this).css('left', left);
			});
		}
		else
		{
			...
			var previousW = $(this).closest('.half').width();
			$('.half').show();
			$(this).closest('.half').show().removeClass('full').addClass('size1of2');
			var newW = $(this).closest('.half').width();
			var left = parseInt($(this).closest('.half').find('.activeImage').css('left'));
			left += (newW - previousW)/2;
			// Current image first
	...
	function updateImages(viewer, range, direction)
	{
		...
		var cur   = series.index;
		var cols  = $(viewer).data('cols');
		//$('#debug').show().html(range+','+cur+','+direction+';');
		$(viewer).find('img').each( function(index){
			dir = false;
			switch (direction)
			{ 
			  case -1: dir = ((index >= cur) && (index <= cur + range)); break;
			  case  0: dir = ((index >= cur - range/2) && (index <= cur + range/2)); break;
			  case  1: dir = ((index >= cur - range) && (index <= cur)); break;
			}
			//$('#debug').append(dir+';');
			if (dir || (index == cur))
			{
				var newSrc = series.seriesImages[index]+'&columns='+cols+'&windowCenter='+wC+'&windowWidth='+wW;
				//$('#debug').append(newSrc+','+$(this).attr('src')+';');
	...
	{
		$('.toolbar, #vReport, #vFull, #'+viewer+' .preset').hide();
		$('#toolImgs').show();
		//alert($('.activeViewer').length);
		if ($('.activeViewer').length == 0)
		{
			$('.half').removeClass('full').addClass('size1of2').show();
		}
		var imagesH = diff - 4;
		var imagesW = $('#'+viewer).width();
		var description = $(elem).next().next().text()

		seriesResponse = {};
		//$('#'+viewer).removeData('series');
		//$('#toolDocs').show();
		$.getScript($(elem).attr('href'), function(data) {
			imageInfoURL = 'admin/ajax/image?study='+seriesResponse.studyUID+'&image=0';
			$.getScript(imageInfoURL, function(info) {
				//alert(imageInfo.wCenter+' '+imageInfo.wWidth);
				$('#'+viewer).data('wCenter', imageInfo.wCenter).data('wWidth', imageInfo.wWidth);

				$('#'+viewer).data('series', seriesResponse);

				//var img = '<div class="rel" style="background:red;width:1px;height:1px;top:0px;left:0px;font-size:1px;z-index:200;">&nbsp;</div>';
				var img = '';
				//$('#debug').show().html();
				for(var i=0; i<seriesResponse.frames;i++)
				{
					//alert(seriesResponse.ratio[i]+ ' ' + imagesW + ' ' + imagesH + ' ' + (imagesW/imagesH));
					// First option is for wide images
					dims = (seriesResponse.ratio[i] >= (imagesW/imagesH)) ? '&columns='+imagesW : '&rows='+imagesH;
					imgClass = (i == 0)? ' activeImage' : ' none';
					var w   = (seriesResponse.ratio[i] >= (imagesW/imagesH)) ? imagesW : imagesW/seriesResponse.ratio[i]; 
					var h   = w/seriesResponse.ratio[i];
					src = (i > 40) ? seriesResponse.seriesImages[0]+dims : seriesResponse.seriesImages[i]+dims;
					src += '&windowWidth='+imageInfo.wWidth+'&windowCenter='+imageInfo.wCenter;
					//left = (seriesResponse.ratio[i] >= (imagesW/imagesH)) ? ((imagesW - w)/2) : 0;
					left = (imagesW - w)/2;
					//$('#debug').append(seriesResponse.ratio[i]+' '+imagesW+' '+imagesH);
					//var itop = (seriesResponse.ratio[i] >= (imagesW/imagesH)) ? ((imagesH - h)/2) : 0;
					var itop = (imagesH - h)/2;
					$('#'+viewer).data('cols', w);
					error = 'onerror="this.onerror=null;this.src=\''+src.replace('wado', 'wado2')+'\';"';
					img += '<img  class="center abs'+imgClass+'" style="left:'+left+'px;top:'+itop+'px;" '+error+' src="'+src+'" />';
					//img += '<img class="center rel'+imgClass+'" onerror="this.onerror=null;this.src=this.src;" src="'+seriesResponse.seriesImages[i]+dims+'" />';				
					
					//tmpImage = new Image();
					//tmpImage.src = seriesResponse.seriesImages[i]+dims;
				}
				//alert(seriesResponse.seriesImages[0]);
				$('#'+viewer).data('src', seriesResponse.seriesImages[0]);
				$('#'+viewer+' .images').html(img);
				
				$('#'+viewer+' .seriesDescription').html($('#studies').find('option:selected').text() + '<br />' +description);
				$('#'+viewer+' .totalImages').html('1 out of '+seriesResponse.frames);
				$('#'+viewer+' .preset span').hide();
				if (seriesResponse.modality == 'CT' || seriesResponse.modality == 'MR')
				{
					$('#'+viewer+' .preset span').show().text('Presets');
				}
				// Add settings for Window/Center
				//$('#'+viewer).data('wCenter', 0).data('wWidth', 0);
			});
			
			/*
			if (parseInt($('#'+viewer+' .images img[display!="none"]').width()) > parseInt($('#'+viewer+' .images').width()))
			{
				$('.viewer').hide();
				$('#'+viewer).removeClass('size1of2').addClass('full').show();
			}
			*/
		});
	}
	...
	{
		var series = $(viewer).data('series');
		var index = parseInt(series.index);
		//$('#debug').show().html(series.index + ' ' + series.frames);
		d = Math.round(d);
		d = (d < -5) ? -5 : ((d > 5) ? 5 : d);
		index -= d;
		d = (d < 0) ? -1 : 1;
		...
		//var img = '<img class="center" src="'+seriesImages[index]+'&rows='+(diff -4)+'" />';
		//$('#vHalfL').html(img);
		
		//$('#vHalfL img').attr('src', seriesImages[index]+'&rows='+(diff -4));
		updateImages(viewer, 21, d);
	...
	$('.preset a').click(function(){
		...
		updateImages(half, 1, 0);
		/*
		$(half).find('img').each( function(index){
			$(this).attr('src', series.seriesImages[index]+'&rows='+h+'&&windowCenter='+values[0]+'&windowWidth='+values[1]);
		});
		*/
