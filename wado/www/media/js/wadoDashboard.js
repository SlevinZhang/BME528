(function( $, window ) {
		...
		*/
		_drag: function(){
			...
			$('.paneLoader').click(function(e) {
				...
				loader.sSeries($this.parent().parent().parent().find('.series'), $this.attr('href').substring(1));
			});
	...
	ui = {
		...
		init: function() {
			...
			$('#toggleMenu').click(ui.click.toogleMenu);
			...
			$('.nav a').click(ui.click.viewerScroll);
			...
			$(window).resize(function() {
				$('.viewer').each(function(i) {
					...
					if ($viewer.find('.activeImage').length > 0)
					{
						...
						$viewer.find('.activeImage, .images img').width(imageW).height(imageH).css({top:top, left:left});
						// Save dimensions back
						$.data(viewerDOM, 'dimensions', {vh:currentH, vw:currentW, h:imageH, w:imageW, t:top, l:left});
					}
				});
			});
		},
		...
		click: {
			viewerScroll: function(e) {
				...
				d = (this.html() == 'Up') ? 1 : -1;
				// Navigate to next
				wadoViewer.scroll($(this).closest('.viewer'), d);
			},
			...
			preset: function(e) {
				...
				var h = $viewer.find('img').eq(index).height();
				// Update the "cover" image to new Preset
				$viewer.find('img').eq(index).attr('src', seriesData.seriesImages[index]+'&windowCenter='+values[0]+'&windowWidth='+values[1]);
				// Update surrounding 5 images
				wadoViewer.update($viewer, 5, 0);
			},
			...
			togglePanes: function(e){
				...
				// Do the transition
				ui.layout.transition(activeLayout, newLayout, target);
			},
			
			activeTool: function(e, className) {
				// Stop everything
				e.stopPropagation();
				e.preventDefault();
			...
			imageTool: function(e) {
				// Common stuff
				ui.click.activeTool(e, '.imageTool');
			...
			layoutTool: function(e) {
				...
				// Apply transition
				ui.layout.transition(oldLayout, newLayout);
			},
			...
			toggleMenu: function(e) {
				...
				$('.viewer').each(function(){
					...
					if ($this.find('.activeImage').length > 0)
					{
						...
						var left = parseInt($this.find('.activeImage').css('left'), 10);
						// Calculate new left
						left += difference;
						// Current image first
		...
		*/
		series: function($elem, viewer) {
			...
			*/
			...
			$.getJSON($elem.attr('href'), ajax.scriptLoaded);
			...
			var ajax = {
				...
				scriptLoaded: function(data) {
					...
					$.data(viewerDOM, 'series', seriesResponse);
					...
					$.getJSON(imageInfoURL, imageInfoLoaded);
				},

				imageInfoLoaded: function(info) {
					// Save the Window Center and Window Width obtained
					$.data(viewerDOM, 'wCenter', imageInfo.wCenter).data('wWidth', imageInfo.wWidth);
					...
					{
						// Load as video
						$.data(viewerDOM, 'mode', 'playback');
					}
					else
					{
						// Load as series
						$.data(viewerDOM, 'mode', 'series');
					}
	...
	// Expose wadoDashboard to the window object
