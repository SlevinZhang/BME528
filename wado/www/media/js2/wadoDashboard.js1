(function( $, window ) {
	...
	ui = {
		...
		init: function() {
			...
			// Preset click
			$('.preset').click(ui.click.presetTab);
		...
		click: {
			...
			presetTab: function(e) {
				...
				// Load series data
				var seriesData = $.data(viewerDOM, 'series');
				if ($(this).find('.preset'+seriesData.modality).css('display') == 'none') {
					// Show the preset (if exists based on modality)
					$this.find('.preset'+seriesData.modality).show();
				} else {
					// Show the preset (if exists based on modality)
					$this.find('.preset'+seriesData.modality).hide();
				}
			},
			preset: function(e) {
				var $this = $(this);
			...
			toggleMenu: function(e) {
				...
				var w = $(this).parent().width(),
					...
					difference = ($this.text() == 'hide') ? 
						parseInt(w/4, 10) : 
						-1*parseInt(w/4, 10);
				var toolw = parseInt($('#toolbar').width())+5;
				var framew = parseInt($('#eformframe').width());
				if ($this.text() == 'hide')
				{
