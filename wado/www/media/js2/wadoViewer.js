(function($) {
	...
	*/
	...
	var wadoViewer = {
		...
		clearfab: function(){
			viewerDOM   = $(".viewer")[0];
			$current   = $(viewerDOM).find('.activeImage');
			...
			wadoViewer.updateareas(viewerDOM,1,index);
			wadoViewer.updatevolume(viewerDOM,1,$viewer);			
...
//			$(fab).parent().hide();
			...
			var mask_url = "mask_thumb/"+$(this).attr('name');
				fabric.Image.fromURL(mask_url, function(oImg) {
				  // scale image down, and flip it, before adding it onto canvas
				  var imgw = $(".canvas-container").width();
				  var imgh = $activeImage.height();
				  oImg.set({left:imgw/2,top:imgh/2,width:imgw*0.3,height:imgh*0.3, opacity:0.5});
				  temfab.add(oImg);
...
//			$(fab).parent().hide();
		...
		*/
		start: function(e) {
			...
			// Handle ROI
			if (mode == 'area2D'){
				var gcolor = 255-$.data(viewerDOM,'areano')*20;
...
//				weights = {fontSize:20, fontWeight:18, stroke:2, spacing:1.3};
			...
			fab.add(areanomark);
			$.data($activeImage[0], 'area2dindex', total-1);
					fab.renderAll();
//					for(var k=0;k<16000000;k=k+4){if(data[k]==0&&data[k+1]==255&&data[k+2]==0) 
//					count++;}
//					alert(area+"mm^2");
						var lesionno = $.data(viewerDOM,'areano');
					var areas = wadoViewer.updateareas(viewerDOM,lesionno,index);
					volumes = wadoViewer.updatevolume(viewerDOM,lesionno,$viewer);
					data=null;
		...
		updateareas: function(viewerDOM,lesionno,index){
					...
					{
						area = wadoViewer.getarea(fab,j,viewerDOM);
						areas[j][index]=area;
					}
					$.data(viewerDOM,'area',areas);
					return areas;
		},
		updatevolume: function(viewerDOM,lesionno,$viewer){
					...
					{
					var volume1=0;
					var frame = $.data(viewerDOM,'total');
		...
		getarea: function(fab,lesionno,viewerDOM){
					var gcolor = 255-lesionno*20;
					var count=0;
