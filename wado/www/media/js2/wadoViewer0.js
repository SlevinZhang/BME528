		...
		*/
		start: function(e) {
			...
			if (mode == 'imgROI'){
				...
				// To get a close area
				wadoViewer.closeloop($viewer, {x:pageX, y:pageY}, 'start');
			}	else{
...
//====================================*/
		...
		*/
			...
			if (status == 'active')
			{
				...
				if (mode == 'imgROI') {
					//To get a closed area
					value = wadoViewer.closeloop($viewer, {x:pageX, y:pageY}, 'end');
		...
		*/
		...
		save: function($viewer){
			...
			$.post("saveFabric.php",{ series_id: dcm_seriesData_UID, dcm_index: index, dcm_fabric: savedDataDLJSON, fab_width:savedWidth, fab_height:savedHeight});
		},
...
//				weights = {fontSize:20, fontWeight:18, stroke:2, spacing:1.3};
	...
	// New Function to enable a closed ROI 
		closeloop: function($viewer, point, mode) {
